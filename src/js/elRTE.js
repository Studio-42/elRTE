(function($) {
	...
	elRTE.prototype.open = function(n) {
		var self = this, d, id = this.id+'-document-'+(this.documents.length+1);
		...
		if (n && n.nodeType == 1) {
			...
			this.documents.push({
				...
				source : n.nodeName == 'TEXTAREA' ? $(n) : $('<textarea name="'+($(n).attr('name')||id)+'" />').val(this.filter($(n).html(), 'html'))
			});
			...
			this.trigger('open').focus(this.documents.length-1);
	...
	elRTE.prototype.focus = function(i) {
		...
		if (this.documents.length) {
			if (typeof(i) == 'undefined') {
				i = this.documents[this.active] ? this.active : 0;
			} else if (!this.documents[i]) {
				i = 0;
			}

			d = this.documents[this.active];
			if (this.active != i) {
				if (d) {
					d.source.is(':visible')	&& this.toggle();
					this.trigger('blur');
				}
				this.active = i;
				d = this.documents[i];
...
this.debug('trigger '+e);
		if (this.load) {
			if (typeof(e) != 'object') {
				e = {type : e, target : this.documents[this.active]};
			}
			if (this.listeners[e.type].length) {
